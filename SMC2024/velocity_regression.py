...
for datasetID in datasetIDs:
    ...
    for i in range(len(performance)):
        if performance[i][0]=='Keyboard1' and performance[i][1]==144:
            inputvelocityset=[]
            for j in range(i):
                if performance[i-j][0]=='Keyboard2' and performance[i-j][1]==144 and len(inputvelocityset)<11:
                    inputvelocityset+=[performance[i-j][3]]
            # print(len(inputvelocityset))
            if len(inputvelocityset)==11:
                outputvelocities+=[performance[i][3]]
                inputvelocities+=[inputvelocityset]
                
            
print(outputvelocities,inputvelocities)

    
reg=lr().fit(inputvelocities,outputvelocities)
print(len(datasetIDs),reg.score(inputvelocities,outputvelocities),reg.coef_,reg.intercept_)
